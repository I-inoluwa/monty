...
{
	char **array = NULL;
	int word_count = get_word_count(str, delim), itr = 0, i = 0, j = 0;
	int *str_len;
	...
	while (str[itr])
	{
		str_len = get_str_len(str, itr, delim);
		if (str_len[0] == 0)
		{
			itr += str_len[1];
			free(str_len);
...
/**
 * get_str_len - gets the length of a string, starting from index ix;
 * @str: string to be split;
...
/**
 * get_word_count - checks if a string contains a delim;
